...
export function Internal() {
  ...
  const createTicketNFT = async () => {
    ...
    const res = await writeContract({
      ...
      args: [metadata.name, metadata.attributes[3].value, metadata.attributes[4].value, tokenURI],
    });
  ...
  const getConcertInfo = async () => {
    ...
    for (var i = 0; i < res.length; i++) {
      ...
      const ooo = await fetch(obj.tokenURI.replace('ipfs://', 'https://gateway.pinata.cloud/ipfs/')).then((res) => res.json());
  ...
  return (
    <>
      ...
      <input className="border 5px" value={userAddr2} onChange={(e) => setUserAddress2(e.target.value)}/>
      <button className="border 5px" onClick={getUserInfo}>Get all user info</button>
      <button className="border 5px" onClick={balanceOf}>Given user tokenId</button>
      <button className="border 5px" onClick={sellTicket}>Buy ticket</button> 
      <p>All user info: {info2}</p>
      <p>Given user's tokenId: {info3}</p>
    </>
  )
}